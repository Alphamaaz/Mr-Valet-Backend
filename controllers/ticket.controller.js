...
const manualCarArrivalSchema = z.object({
  ...
  entryMethod: z.enum(["CAMERA", "NFC", "SERIALIZED_PAPER", "PRINTER", "SMS"]).default("CAMERA"),
  vehicle: z.object({
    plate: z.string().trim().min(2).max(20),
...
export async function createTicket(req, res) {
  ...
  if (data.locationId && !isValidObjectId(data.locationId)) {
    throw badRequest("locationId must be a valid ObjectId");
  }

  if (data.ownerUserId && !isValidObjectId(data.ownerUserId)) {
    throw badRequest("ownerUserId must be a valid ObjectId");
  }
  ...
  const ticket = await Ticket.create({
    ...
    valetCode: generateValetCode(),
    ownerType: data.ownerType,
    ownerPhone: data.ownerPhone || "",
    ownerUser: data.ownerUserId || null,
...
export async function createManualCarArrival(req, res) {
  ...
  const qrContent = ownerType === OWNER_TYPES.WHATSAPP ? whatsappPrefillLink : ticket.valetCode;
  const qrCodeDataUrl = await QRCode.toDataURL(qrContent, { margin: 1, width: 280 });
    ...
    new ApiResponse(
      201,
      {
        ...
        ownerFlow: {
          ownerType,
          whatsappCommand,
          whatsappPrefillLink: ownerType === OWNER_TYPES.WHATSAPP ? whatsappPrefillLink : "",
